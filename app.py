...
few-shot 数据集：/Dataset/Voxceleb1/voxceleb1_5shot3way/
代码：/home/ps/3D-Speaker/few-shot-OOD-speaker-verification-main
        ...
        with vis_col2:
            if show_umap and sel_family:
                ...
                if sel_result and "similarity_matrix" in sel_result:
                    ...
                    # 用相似度矩阵的行作为嵌入近似展示
                    embs = sim_mat.detach().cpu().numpy() if hasattr(sim_mat, "detach") else np.asarray(sim_mat)
                    labels = true_labels.detach().cpu().numpy() if hasattr(true_labels, "detach") else np.asarray(true_labels)
                    ood_mask = labels < 0
        ...
        with heat_col2:
            if show_heat and heat_family:
                ...
                if hr and "similarity_matrix" in hr:
                    sim_mat_obj = hr["similarity_matrix"]
                    true_labels_obj = hr["true_labels"]
                    sim_mat = sim_mat_obj.detach().cpu().numpy() if hasattr(sim_mat_obj, "detach") else np.asarray(sim_mat_obj)
                    true_labels_np = true_labels_obj.detach().cpu().numpy() if hasattr(true_labels_obj, "detach") else np.asarray(true_labels_obj)
                    # 按真实标签排序
